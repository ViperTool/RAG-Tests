...
Важно! С последних изменений на сайте вики элементы не подгружаются с помощью статистических парсеров, поэтому в проекте используется Selenium. Для его использования необходим браузер Chrome на компьютере:
```
python cli.py parse
```
#### 3. Чанкирование
