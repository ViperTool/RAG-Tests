...
*   **LLM & Embeddings:** Использует `Qwen3-1.7B` (генерация) и `ai-forever/FRIDA` (эмбеддинги).
*   **Оптимизация памяти:** Раздельная загрузка моделей (CPU/GPU) для запуска на картах с небольшим количеством памяти.
*   **Умный поиск:** Собственная реализация ранжирования чанков, объединяющая лексический и семантический поиск.
