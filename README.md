...
### Особенности
*   **Стек:** Python 3.10+, PyTorch, Transformers, ChromaDB, SQLite, BeautifulSoup4, Selenium, spaCy.
...
*   **Полная настраиваемость**: При желании можно выбрать другую Вики, другую модель для генерации и эмбеддингов, настроить названия коллекций, изменить системный промпт, выбрать предпочтительную стратегию чанкирования и так далее с помощью понятного config.py.
...
Важно! С последних изменений на сайте вики элементы не подгружаются с помощью статических парсеров, поэтому в проекте используется Selenium. Для его использования необходим браузер Chrome на компьютере:
```
python cli.py parse
```
#### 3. Чанкирование
Создание таблицы с чанками в SQLite. Аргумент -s отвечает за стратегию чанкирования: 'fixed' (значение по умолчанию; фиксированная с перекрытием) или 'sliding' (плавающее окно)
```
python cli.py database
...
Важно! Для использования удалённых API в корне необходимо создать файл с секретами ".env", в котором передать единственную строку:
```G_REMOTE_MODEL_API = "Ваш API с OpenRouter"```