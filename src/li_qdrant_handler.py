...
def make_qdrant():
    PDF_PATH = "./data/Russia.pdf"  # Путь к вашему PDF файлу
    ...
    def get_embedding(text: str) -> list[float]:
        # Гарантируем, что text — непустая строка
    ...
    for doc_idx, doc in enumerate(tqdm.tqdm(documents, desc="Обработка документов")):
        # Защита от None или пустого текста
        if doc.text is None:
            continue
        ...
        # Обновляем metadata, добавляя информацию о документе
        doc.metadata = {"page_id": doc.metadata.get("page_label", f"pdf_{doc_idx}"), **doc.metadata}

        try:
            nodes = splitter.get_nodes_from_documents([doc])
        except Exception as e:
            print(f"⚠️ Ошибка чанкирования для документа {doc_idx}: {e}")
            continue
        ...
        for node in nodes:
            ...
            points.append(PointStruct(
                ...
                payload={
                    ...
                    "text": chunk_text,  # ← теперь LlamaIndex сможет найти текст
                    "chunk_index": node.node_id,
                    "pdf_document": PDF_PATH  # Добавляем путь к исходному PDF
                }
            ))
            point_id_counter += 1
