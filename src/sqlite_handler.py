...
def create_content(table_name: str, db_path: str = "data/wiki_content.db"):
    """
    ...
    conn.commit()
    conn.close()
    print(f"База данных содержимого и чанков создана: {db_path}, наименование таблицы: {table_name}")


def save_or_update_page_by_url(url: str, content: str, table_name: str, db_path: str = "data/wiki_content.db") -> Optional[int]:
    """
    ...
    try:
        cursor.execute(
            f"INSERT OR REPLACE INTO {table_name} (url, content) VALUES (?, ?)",
            (url, content)
        )
...
def save_or_update_page_by_id(page_id: int, content: str, table_name: str, db_path: str = "data/wiki_content.db") -> Optional[bool]:
    """
...
def save_chunks(page_id: int, chunks: List[str], table_name: str, db_path: str = "data/wiki_content.db"):
    """
    ...
    try:
        ...
        for i, chunk_text in enumerate(chunks):
            cursor.execute(
                f"INSERT INTO {table_name} (page_id, chunk_text, chunk_order) VALUES (?, ?, ?)",
...
def get_all_pages(table_name: str, db_path: str = "data/wiki_content.db") -> List[Tuple[int, str, str]]:
    """
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute(f"SELECT id, url, content FROM {table_name}")
    results = cursor.fetchall()
    conn.close()
    return results


def get_all_chunks(table_name: str, db_path: str = "data/wiki_content.db") -> List[Tuple[int, int, str, int]]:
    """
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute(f"SELECT chunk_id, page_id, chunk_text, chunk_order FROM {table_name} ORDER BY page_id, chunk_order")
    results = cursor.fetchall()
    conn.close()
    return results


def get_page_by_url(url: str, table_name: str, db_path: str = "data/wiki_content.db") -> Tuple[int, str, str] | None:
    """
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute(f"SELECT id, url, content FROM {table_name} WHERE url = ?", (url,))
    result = cursor.fetchone()
    conn.close()
    return result


def get_page_by_id(page_id: int, table_name: str, db_path: str = "data/wiki_content.db") -> Tuple[int, str, str] | None:
    """
...
def get_chunks_by_page_id(page_id: int, table_name: str, db_path: str = "data/wiki_content.db") -> List[Tuple[int, int, str, int]]:
    """
    ...
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute(f"SELECT chunk_id, page_id, chunk_text, chunk_order FROM {table_name} WHERE page_id = ? ORDER BY chunk_order", (page_id,))
    results = cursor.fetchall()
    conn.close()
    return results


def delete_page_and_chunks(page_id: int, table_name: str, delete_original: bool = False, db_path: str = "data/wiki_content.db"):
    """
    ...
    try:
        ...
        if delete_original:
            cursor.execute(f"DELETE FROM {table_name} WHERE id = ?", (page_id,))
...
def clear_db(table_name: str, db_path: str = "data/wiki_content.db", clear_pages: bool = False):
    """
...
def form_chunks_swr(title: str, content: str) -> List[str]:
    ...
    # Формируем предложения, объединяя текст и следующий за ним разделитель
    sentences = [
        parts[i] + parts[i + 1]
        for i in range(0, len(parts) - 1, 2)
        if parts[i] or parts[i + 1]  # Пропускаем пустые элементы
    ]

    # Добавляем префикс к каждому предложению
    prefix = f'search_document: {title} '
    total_chunks = [prefix + sentence for sentence in sentences]

    # Если не получилось ни одного чанка - возвращаем базовый префикс
...
def make_table(table_name: str, db_path: str = "data/wiki_content.db"):
    """
    ...
    create_content(table_name=table_name)
    ...
    for page in pages:
        ...
        save_chunks(page_id=page[0], chunks=chunks, table_name=table_name, db_path=db_path)

# Пример работы

if __name__ == '__main__':
    make_table('wiki_pages_swr')
